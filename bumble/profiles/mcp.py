...
class MediaControlServiceProxy(
    ...
    async def write_control_point(
        ...
        Raises:
            UnsupportedOperationError: Server does not have Media Control Point Characteristic.
            InvalidStateError: Server replies a notification with mismatched opcode.
        '''
        if not self.media_control_point:
            raise core.UnsupportedOperationError(
                "Peer does not have media control point"
            )
