...
class PlayingOrder(enum.IntEnum):
    '''
...
class MediaControlPointOpcode(enum.IntEnum):
    '''
...
class SearchControlPointItemType(enum.IntEnum):
    '''
...
class ObjectId(int):
    @classmethod
    def parse_from(cls: Type[Self], data: bytes) -> Self:
...
class GroupObjectType:
    ...
    def parse_from(cls: Type[Self], data: bytes) -> Self:
        return cls(
            ...
            object_id=ObjectId.parse_from(data[1:]),
        )
...
class MediaControlServiceProxy(
    ...
    def __init__(self, service_proxy: gatt_client.ServiceProxy) -> None:
        ...
        self.media_control_point_notifications = asyncio.Queue[bytes]()
    ...
    async def write_control_point(
        ...
        Raises:
            InvalidStateError: Server does not have Media Control Point Characteristic.
            ValueError: Server replies a notification with mismatched opcode.
        '''
        if not self.media_control_point:
            raise core.InvalidStateError("Peer does not have media control point")

        async with self.lock:
            self.media_control_point.write_value(bytes([opcode]), with_response=False)
            (
                response_opcode,
                response_code,
