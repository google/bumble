...
class RangingService(gatt.TemplateService):
    ...
    @dataclasses.dataclass
    class Client:
        active_mode: RangingService.Mode
        cccd_value: gatt.ClientCharacteristicConfigurationBits
        # procedure counter to ranging data
        ranging_data_table: dict[int, RangingData] = dataclasses.field(
            default_factory=dict
        )
        # config id to procedure counter
        active_procedure_counter: dict[int, int] = dataclasses.field(
            default_factory=dict
        )
    ...
    real_time_ranging_data_characteristic: gatt.Characteristic[bytes] | None
                    ...
                    gatt.Descriptor(
                        ...
                        value=gatt.AttributeValue(
                            read=functools.partial(
                                self._on_cccd_read, self.Mode.REAL_TIME
                            ),
                            write=functools.partial(
                                self._on_cccd_write, self.Mode.REAL_TIME
                            ),
                        ),
                    )
                ],
        ...
        self.on_demand_ranging_data_characteristic = gatt.Characteristic[bytes](
            ...
            descriptors=[
                gatt.Descriptor(
                    ...
                    value=gatt.AttributeValue(
                        read=functools.partial(self._on_cccd_read, self.Mode.ON_DEMAND),
                        write=functools.partial(
                            self._on_cccd_write, self.Mode.ON_DEMAND
                        ),
                    ),
                )
            ],
    ...
    def _on_cccd_write(
        ...
        if not (client := self.clients.get(connection)):
            client = self.clients[connection] = self.Client(
                active_mode=self.Mode.INACTIVE,
                cccd_value=gatt.ClientCharacteristicConfigurationBits.DEFAULT,
            )

        if client.active_mode not in (self.Mode.INACTIVE, mode):
        ...
        if cccd_value == gatt.ClientCharacteristicConfigurationBits.DEFAULT:
            client.active_mode = self.Mode.INACTIVE
        else:
    ...
    def _post_subevent_result(
        ...
        if event.procedure_done_status == hci.CsDoneStatus.ALL_RESULTS_COMPLETED:
            ...
            if client.active_mode == self.Mode.REAL_TIME:
                connection.abort_on(
                    'disconnection',
    ...
    async def send_ranging_data(
        ...
        if client.active_mode == self.Mode.ON_DEMAND:
            characteristic = self.on_demand_ranging_data_characteristic
        elif client.active_mode == self.Mode.REAL_TIME:
...
class RangingServiceProxy(gatt_client.ProfileServiceProxy):
    ...
    ras_features_characteristic: gatt_client.CharacteristicProxy[bytes]
    on_demand_ranging_data_characteristic: gatt_client.CharacteristicProxy[bytes]
    ras_control_point_characteristic: gatt_client.CharacteristicProxy[bytes]
    ranging_data_ready_characteristic: gatt_client.CharacteristicProxy[bytes]
    ranging_data_overwritten_characteristic: gatt_client.CharacteristicProxy[bytes]

    # Optional.
    real_time_ranging_data_characteristic: (
