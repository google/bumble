...
class L2CAP_Control_Frame:
    ...
    def __init__(self, pdu: Optional[bytes] = None, **kwargs) -> None:
        ...
        self.parameters = pdu[4:] if pdu else b''

    @property
    def parameters(self) -> bytes:
        if self._parameters is None:
            self._parameters = hci.HCI_Object.dict_to_bytes(self.__dict__, self.fields)
    ...
    def __bytes__(self) -> bytes:
        return (
            struct.pack('<BBH', self.code, self.identifier, len(self.parameters) + 4)
            + self.parameters
        )
