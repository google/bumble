...
async def test_subscribe_on_demand_cccd() -> None:
    ...
    assert server.clients[server_connection].active_mode == rap.Mode.ON_DEMAND
    assert (
    ...
    assert server.clients[server_connection].active_mode == rap.Mode.INACTIVE
...
async def test_subscribe_real_time_cccd() -> None:
    ...
    assert server.clients[server_connection].active_mode == rap.Mode.REAL_TIME
    assert (
        server.clients[server_connection].cccd_value
        == gatt.ClientCharacteristicConfigurationBits.NOTIFICATION
    )
