...
class SpectableEnum(utils.OpenIntEnum):
    ...
    @classmethod
    def metadata(cls, size: int, list_begin: bool = False, list_end: bool = False):
...
class CsDoneStatus(SpectableEnum):
    ALL_RESULTS_COMPLETED = 0x00
    PARTIAL = 0x01
    ABORTED = 0x0F


class CsProcedureAbortReason(SpectableEnum):
    NO_ABORT = 0x00
...
class CsSubeventAbortReason(SpectableEnum):
    NO_ABORT = 0x00
    LOCAL_HOST_OR_REMOTE_REQUEST = 0x01
...
# Address types
class AddressType(SpectableEnum):
    PUBLIC_DEVICE   = 0x00
    RANDOM_DEVICE   = 0x01
...
class LmpFeature(SpectableEnum):
...
# -----------------------------------------------------------------------------
class OwnAddressType(SpectableEnum):
    PUBLIC = 0
    RANDOM = 1
...
# -----------------------------------------------------------------------------
class LoopbackMode(SpectableEnum):
    DISABLED = 0
    LOCAL = 1
    REMOTE = 2
...
class HCI_Enhanced_Setup_Synchronous_Connection_Command(HCI_Command):
    ...
    class PcmDataFormat(SpectableEnum):
        NA = 0x00
    ...
    class DataPath(SpectableEnum):
        HCI = 0x00
        PCM = 0x01

    class RetransmissionEffort(SpectableEnum):
...
class HCI_LE_CS_Create_Config_Command(HCI_Command):
    ...
    class ChannelSelectionType(SpectableEnum):
        ALGO_3B = 0
        ALGO_3C = 1

    class Ch3cShape(SpectableEnum):
        HAT = 0x00
        X = 0x01
...
class HCI_LE_Connection_Complete_Event(HCI_LE_Meta_Event):
    ...
    role: int = field(metadata=Role.metadata(1))
    peer_address_type: int = field(metadata=AddressType.metadata(1))
...
class HCI_LE_PHY_Update_Complete_Event(HCI_LE_Meta_Event):
    ...
    status: int = field(metadata=metadata(STATUS_SPEC))
    connection_handle: int = field(metadata=metadata(2))
    tx_phy: int = field(metadata=Phy.metadata(1))
    rx_phy: int = field(metadata=Phy.metadata(1))
...
class HCI_LE_Periodic_Advertising_Sync_Established_Event(HCI_LE_Meta_Event):
    ...
    sync_handle: int = field(metadata=metadata(2))
    advertising_sid: int = field(metadata=metadata(1))
    advertiser_address_type: int = field(metadata=AddressType.metadata(1))
    advertiser_address: int = field(
        metadata=metadata(Address.parse_address_preceded_by_type)
    )
    advertiser_phy: int = field(metadata=Phy.metadata(1))
    periodic_advertising_interval: int = field(metadata=metadata(2))
...
class HCI_LE_Periodic_Advertising_Report_Event(HCI_LE_Meta_Event):
    ...
    class DataStatus(SpectableEnum):
        DATA_COMPLETE = 0x00
        DATA_INCOMPLETE_MORE_TO_COME = 0x01
        DATA_INCOMPLETE_TRUNCATED_NO_MORE_TO_COME = 0x02
    ...
    tx_power: int = field(metadata=metadata(-1))
    rssi: int = field(metadata=metadata(-1))
    cte_type: int = field(metadata=CteType.metadata(1))
    data_status: int = field(metadata=DataStatus.metadata(1))
    data: bytes = field(metadata=metadata("v"))
...
class HCI_LE_Periodic_Advertising_Report_V2_Event(HCI_LE_Meta_Event):
    ...
    rssi: int = field(metadata=metadata(-1))
    cte_type: int = field(
        metadata=metadata(HCI_LE_Periodic_Advertising_Report_Event.CteType.metadata(1))
    )
...
class HCI_LE_BIGInfo_Advertising_Report_Event(HCI_LE_Meta_Event):
    ...
    phy: int = field(metadata=Phy.metadata(1))
...
class HCI_LE_CS_Config_Complete_Event(HCI_LE_Meta_Event):
    ...
    action: int = field(metadata=Action.metadata(1))
...
class HCI_LE_CS_Procedure_Enable_Complete_Event(HCI_LE_Meta_Event):
    ...
    connection_handle: int = field(metadata=metadata(2))
    config_id: int = field(metadata=metadata(1))
    state: int = field(metadata=State.metadata(1))
...
class HCI_QOS_Setup_Complete_Event(HCI_Event):
    ...
    class ServiceType(SpectableEnum):
